...
### 说明  
基于互联网的应用，无论用户如何变化与位置分散，所有的信息与交流都会汇集于服务器，这在无形之中也不可避免地造成了服务端的利维坦化。即服务提供方将接管所有用户的兴趣，需求与内容。这可能会导致普通人的信息污染与泄漏，同时导致不安全的事情发生。正常的公民生活需要“自我—管理(self－management)、交互—公共领域合作(inter－public coordination)以及政治责任(political accountability)” --弗雷泽.其含义是希望每个个体能成为自我管理，自我维护与分发的数据节点。设计这一款简单的服务框架的初衷，就是将服务器的部署与运行尽量解放出来由用户来自己控制(当然可能需要一台云服务器)，从而尽量减少在一些在次群(sub group)交流过程中导致的隐私信息泄漏.目前这是服务端框架，尽力提供了简单部署，简单迁移，简单恢复，谋求用最小的成本来实现简单化的群聊系统.这只是提供了另外一种小的可能.

### 客户端
* 在 https://github.com/nmsoccer/schat-cli/releases 下载客户端进行功能验证
...
### 命令行演示
  ...
  * 连接地址  
    * 方法一：我们的dir_serv默认监听于11802端口，所以可以使用浏览器打开xxx:11802/query页面获得一个conn_serv的地址：
    ```
...
please input:>>
  ...
  * 文件  
  以图片为例，在群内发送图片也会作为群聊信息的一条进行同步，同步的是生成的url数据。
    * 地址  
