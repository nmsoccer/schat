...
func HandleClientPkg(pconfig *Config, pclient *comm.ClientPkg) {
	...
	if uid <= 0 { //only some proto no need uid
	...
	case cs.CS_PROTO_CREATE_GRP_REQ:
		pmsg, ok := gmsg.SubMsg.(*cs.CSCreateGroupReq)
		if ok {
			SendCreateGroupReq(pconfig, uid, pmsg)
			conv_err = false
		}
	case cs.CS_PROTO_APPLY_GRP_REQ:
		pmsg, ok := gmsg.SubMsg.(*cs.CSApplyGroupReq)
		if ok {
			SendApplyGroupReq(pconfig, uid, pmsg)
			conv_err = false
		}
	...
	case cs.CS_PROTO_APPLY_GRP_AUDIT:
		pmsg, ok := gmsg.SubMsg.(*cs.CSApplyGroupAudit)
		if ok {
			SendApplyGroupAudit(pconfig, uid, pmsg)
			conv_err = false
		}
	case cs.CS_PROTO_SEND_CHAT_REQ:
		pmsg, ok := gmsg.SubMsg.(*cs.CSSendChatReq)
		if ok {
	...
	case cs.CS_PROTO_KICK_GROUP_REQ:
		pmsg, ok := gmsg.SubMsg.(*cs.CSKickGroupReq)
		if ok {
	...
	case cs.CS_PROTO_CHG_GROUP_ATTR_REQ:
		pmsg, ok := gmsg.SubMsg.(*cs.CSChgGroupAttrReq)
		if ok {
			SendChgGroupAttrReq(pconfig, uid, pmsg)
			conv_err = false
		}
	case cs.CS_PROTO_GROUP_GROUND_REQ:
		pmsg, ok := gmsg.SubMsg.(*cs.CSGroupGroundReq)
		if ok {
